...
const PAGE_SIZE = 20;
const BASE_PATH = "/dashboard/viewings";
...
export default async function ViewingsPage({ searchParams }: PageProps) {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="font-serif text-2xl text-foreground">Viewing Requests</h1>
                <p className="mt-1 text-sm text-muted-foreground">
                    {total} viewing request{total === 1 ? "" : "s"} from property pages
                </p>
            </div>

            <LeadTableToolbar />
            <LeadTable items={items} emptyMessage="No viewing requests match these filters." />
